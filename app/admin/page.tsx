...
// ---------------------------------------------------------------------------
// Shared style tokens (matches the existing admin dashboard)
// ---------------------------------------------------------------------------
...
function Modal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-center bg-stone-900/40 p-0 sm:items-center sm:p-4 dark:bg-stone-950/70"
      onClick={onClose}
    >
      <div
        className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-t-2xl bg-white p-5 shadow-xl sm:rounded-2xl dark:bg-[#1a3a28]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-base font-semibold text-stone-800 dark:text-[#d4f5e0]">{title}</h3>
          <button
            onClick={onClose}
            className="rounded-lg p-1.5 text-stone-400 hover:bg-stone-100 hover:text-stone-600 dark:hover:bg-[#0a1f14] dark:hover:text-[#a3c9b8]"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
...
function EmptyState({ title, hint }: { title: string; hint: string }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-stone-300 bg-white/60 py-16 text-center dark:border-[#2a4a38] dark:bg-[#1a3a28]/60">
      <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-[#F1E4C3] dark:bg-[#2a4a38]">
        <Newspaper className="h-5 w-5 text-[#9A6A11] dark:text-[#a3c9b8]" />
      </div>
      <p className="font-medium text-stone-700 dark:text-[#a3c9b8]">{title}</p>
      <p className="mt-1 max-w-xs text-sm text-stone-400 dark:text-[#8ab5a0]">{hint}</p>
    </div>
...
const emptyDraft: ArticleInput = { title: "", content: "" };
...
export default function ArticlesAdmin() {
  ...
  const [formModal, setFormModal] = useState<{ open: boolean; id: string | null; draft: ArticleInput }>({
    open: false,
    id: null,
    draft: emptyDraft,
  });
  ...
  async function load() {
    ...
    try {
      ...
      console.error(err);
      setError("Couldn't load articles from Appwrite. Check your database/collection IDs and permissions.");
    } finally {
      setLoading(false);
    }
  }
  ...
  function openCreate() {
    setFormModal({ open: true, id: null, draft: emptyDraft });
  }
  ...
  function openEdit(a: Article) {
    setFormModal({
      open: true,
      id: a.$id,
      draft: { title: a.title, content: a.content, featured_image: a.featured_image },
    });
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      if (formModal.id) {
        const updated = await updateArticle(formModal.id, formModal.draft);
        setArticles((prev) => prev.map((a) => (a.$id === updated.$id ? updated : a)));
      } else {
        const created = await createArticle(formModal.draft);
        setArticles((prev) => [created, ...prev]);
      }
      setFormModal({ open: false, id: null, draft: emptyDraft });
    } catch (err) {
      console.error(err);
      alert("Couldn't save the article. Check the console for details.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id: string) {
    try {
      ...
      console.error(err);
      alert("Couldn't delete the article. Check the console for details.");
    }
  }

  return (
    <div className="min-h-screen w-full  bg-[#F6F4EF] p-4 text-stone-800 sm:p-6 dark:bg-[#0a1f14] dark:text-[#a3c9b8]">
      {/* Header */}
      <div className="mb-5 flex flex-col mt-24 lg:mt-16 gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-lg font-semibold text-stone-800 dark:text-[#d4f5e0]">Articles</h1>
          <p className="text-xs text-stone-400 dark:text-[#8ab5a0]">Backed by your Appwrite collection.</p>
        </div>
      ...
      {error && (
        <div className="mb-4 flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-300">
          ...
          <div>
            <p>{error}</p>
            <button onClick={load} className="mt-1 font-medium underline underline-offset-2">
              Try again
            </button>
          </div>
        </div>
      ...
      {/* Loading */}
      {loading && (
        <div className="flex items-center justify-center gap-2 rounded-2xl border border-stone-200 bg-white py-16 text-sm text-stone-400 dark:border-[#2a4a38] dark:bg-[#1a3a28] dark:text-[#8ab5a0]">
      ...
      {!loading && !error && filtered.length === 0 && (
        <EmptyState title="No articles yet" hint="Create your first article to see it here." />
      )}

      {/* Table */}
      {!loading && !error && filtered.length > 0 && (
        <div className="overflow-hidden rounded-2xl border border-stone-200 bg-white dark:border-[#2a4a38] dark:bg-[#1a3a28]">
          <div className="hidden grid-cols-[1fr_110px_90px_90px] gap-3 border-b border-stone-100 px-4 py-2.5 text-xs font-medium text-stone-400 sm:grid dark:border-[#2a4a38] dark:text-[#8ab5a0]">
            <span>Title</span>
            <span>Date</span>
            <span />
          </div>
          <div className="divide-y divide-stone-100 dark:divide-[#2a4a38]">
            {filtered.map((a) => (
              <div
                key={a.$id}
                className="grid grid-cols-1 gap-2 px-4 py-3 sm:grid-cols-[1fr_110px_90px_90px] sm:items-center sm:gap-3"
              >
                <button
                  onClick={() => setDetailArticle(a)}
                  className="truncate text-left text-sm font-medium text-stone-700 hover:underline dark:text-[#a3c9b8]"
                >
                  {a.title}
                </button>
                <p className="text-xs text-stone-400 dark:text-[#8ab5a0]">{new Date(a.$createdAt || "").toLocaleDateString() ?? "—"}</p>
                <div className="flex gap-1 sm:justify-end">
                  <button
                    onClick={() => setDetailArticle(a)}
                    className="rounded-md p-1.5 text-stone-400 hover:bg-stone-100 hover:text-stone-600 dark:hover:bg-[#0a1f14] dark:hover:text-[#a3c9b8]"
                    title="View details"
                  >
                  ...
                  <button
                    ...
                    className="rounded-md p-1.5 text-stone-400 hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-950/50 dark:hover:text-red-400"
                    title="Delete"
                  >
                    <Trash2 className="h-3.5 w-3.5" />
                  </button>
      ...
      <Modal
        open={formModal.open}
        onClose={() => setFormModal({ open: false, id: null, draft: emptyDraft })}
        title={formModal.id ? "Edit article" : "New article"}
      >
        <form className="space-y-3" onSubmit={handleSave}>
          <div>
            ...
            <input
              required
              className={inputCls}
              value={formModal.draft.title}
              onChange={(e) =>
                setFormModal((m) => ({ ...m, draft: { ...m.draft, title: e.target.value } }))
              }
            />
          </div>
          <div>
            ...
            <textarea
              ...
              rows={6}
              className={inputCls}
              value={formModal.draft.content}
              onChange={(e) =>
                setFormModal((m) => ({ ...m, draft: { ...m.draft, content: e.target.value } }))
              }
            />
          </div>
          <div>
            ...
            <input
              ...
              value={formModal.draft.featured_image ?? ""}
              onChange={(e) =>
                setFormModal((m) => ({ ...m, draft: { ...m.draft, featured_image: e.target.value } }))
              }
            />
          </div>
          <button
            type="submit"
            disabled={saving}
            className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#181A20] py-2.5 text-sm font-medium text-white hover:bg-stone-800 disabled:opacity-60"
          >
            {saving && <Loader2 className="h-4 w-4 animate-spin" />}
            {formModal.id ? "Save changes" : "Create article"}
          </button>
        </form>
      </Modal>

      {/* Details modal */}
      <Modal open={!!detailArticle} onClose={() => setDetailArticle(null)} title={detailArticle?.title ?? ""}>
        {detailArticle && (
          <div className="space-y-4">
            {detailArticle.featured_image && (
              <img
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs text-stone-400 dark:text-[#8ab5a0]">
              {detailArticle.$createdAt && (
                <span className="flex items-center gap-1">
                  <SquarePen className="h-3 w-3" /> {new Date(detailArticle.$createdAt).toLocaleDateString()}
                </span>
              )}
              {detailArticle.$updatedAt && (
                <span className="flex items-center gap-1">
                  <Calendar className="h-3 w-3" /> {new Date(detailArticle.$updatedAt).toLocaleDateString()}
                </span>
              )}
            </div>
            <p className="whitespace-pre-wrap text-sm leading-relaxed text-stone-600 dark:text-[#a3c9b8]">
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-stone-200 py-2 text-sm font-medium text-stone-600 hover:bg-stone-50 dark:border-[#2a4a38] dark:text-[#a3c9b8] dark:hover:bg-[#0a1f14]"
              >
                <Pencil className="h-3.5 w-3.5" /> Edit
              </button>
              <button
                onClick={() => setConfirmDeleteId(detailArticle.$id)}
                className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-red-200 py-2 text-sm font-medium text-red-500 hover:bg-red-50 dark:border-red-900/50 dark:hover:bg-red-950/30"
              >
      ...
      <Modal open={!!confirmDeleteId} onClose={() => setConfirmDeleteId(null)} title="Delete article?">
        <div className="space-y-4">
          <p className="text-sm text-stone-500 dark:text-[#8ab5a0]">
            This can't be undone. The article will be permanently removed from Appwrite.
          </p>
          <div className="flex gap-2">
            <button
              onClick={() => setConfirmDeleteId(null)}
              className="flex-1 rounded-lg border border-stone-200 py-2 text-sm font-medium text-stone-600 hover:bg-stone-50 dark:border-[#2a4a38] dark:text-[#a3c9b8] dark:hover:bg-[#0a1f14]"
            >
              Cancel
            </button>
            <button
